...
fn repeat_str(original_str: &str, num: u16) -> String {
...
fn print_text_at(text: &str, coords: (i16, i16), bounds: (u16, u16), term_size: (u16, u16)) {
...
fn print_text_at_with_color(text: &str, coords: (u16, u16), color: &dyn termion::color::Color, bounds: (u16, u16), term_size: (u16, u16)) {
...
fn draw_border(bounds: (u16, u16), term_size: (u16, u16)) {
	let (w, h) = bounds;
	// Draw corners
...
fn clear_screen(terminal_height: u16) {
...
fn get_initial_state() -> GameState {
	...
	let bounds = (std::cmp::min(w-3, 30), std::cmp::min(h-3, 10));
	let bodies = vec![(1, 1)];
	GameState {
